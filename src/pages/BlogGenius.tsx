...
import { Calendar, User, Activity, Hand, AlignVerticalJustifyCenter, Heart, Phone, Mail, Eye, EyeOff } from 'lucide-react';
...
// Enhanced BlogCard with visual distinction for hidden articles
const BlogCardGenius = React.memo(({ post, showHiddenBadge }: { post: BlogPost; showHiddenBadge?: boolean }) => {
    const isHidden = post.featured === false;
        ...
        <article className={`flex flex-col border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all ${isHidden
            ...
            <Link to={`/blog/${post.slug}`} className="block h-48 overflow-hidden relative">
                {isHidden && showHiddenBadge && (
                    <div className="absolute top-2 right-2 z-10 bg-orange-500 text-white px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 shadow-md">
                        <EyeOff size={14} />
                        MASQUÉ
                    </div>
                )}
                {isCloudinaryImage(post.image) ? (
                    <AdvancedImage
                        ...
                        className={`w-full h-full object-cover transition-transform duration-500 hover:scale-105 ${isHidden ? 'grayscale' : ''}`}
                        alt={post.title}
                    />
                ) : (
                    <img
            ...
            <div className="p-6 flex flex-col flex-grow">
                ...
                <Link to={`/blog/${post.slug}`} className="group">
                    <h2 className={`text-xl font-bold mb-3 group-hover:text-primary transition-colors ${isHidden ? 'text-gray-600' : 'text-text-main'
                        }`}>
                        {post.title}
                    </h2>
                </Link>
...
BlogCardGenius.displayName = 'BlogCardGenius';


export const BlogGenius: React.FC = () => {
    ...
    // Count articles per category (including hidden)
    ...
    // Count hidden articles per category
    ...
    // Filter posts - SHOW ALL articles for admin view
    const filteredPosts = useMemo(() => {
        let posts = [...BLOG_POSTS];

        // Filter by hidden status if checkbox is active
        if (showOnlyHidden) {
            posts = posts.filter((post) => post.featured === false);
        }
    ...
    const hiddenCount = BLOG_POSTS.filter(p => p.featured === false).length;
    const totalCount = BLOG_POSTS.length;

    return (
        <>
            <SEO
                title="Blog Genius - Administration"
                description="Interface d'administration pour gérer la visibilité des articles de blog."
            />

            <div className="py-6 md:py-8 bg-gradient-to-br from-amber-50 to-orange-50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6">
                        <div>
                            <span className="inline-block px-3 py-1 bg-gradient-to-r from-orange-500 to-amber-500 text-white text-xs font-bold rounded-full mb-4">
                                🔒 ADMIN - BLOG GENIUS
                            </span>
                            <SectionHeader
                                ...
                                title={
                                    <>
                                        Gestion des articles <br /> <span className="text-gradient-primary">Batignolles Kiné Sport</span>
                                    </>
                                }
                                description={`Visualisez et gérez tous les articles. ${totalCount} au total, dont ${hiddenCount} masqué${hiddenCount > 1 ? 's' : ''}.`}
                            />
                        </div>
                    </div>

                    {/* Filter Controls */}
                    <div className="mb-6 bg-white rounded-lg shadow-sm border-2 border-orange-200 p-4">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <label className="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span className="text-sm font-semibold text-gray-700 group-hover:text-orange-600 transition-colors flex items-center gap-2">
                                    ...
                                    Afficher uniquement les articles masqués ({hiddenCount})
                                </span>
                            </label>
                            ...
                            {(activeCategory || showOnlyHidden) && (
                                <button
                                    ...
                                    className="text-sm text-orange-600 hover:text-orange-700 font-semibold transition-colors underline"
                                >
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div id="articles-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {paginatedPosts.map((post) => (
                            <BlogCardGenius key={post.slug} post={post} showHiddenBadge={true} />
                        ))}
                    ...
                    {filteredPosts.length === 0 && (
                        <div className="text-center py-16">
                            <p className="text-text-muted text-lg mb-4">Aucun article {showOnlyHidden ? 'masqué' : ''} trouvé {activeCategory ? 'dans cette catégorie' : ''}.</p>
                            <button
                                onClick={handleResetFilters}
